...
export function DispatchArchiveDetail() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { toast } = useToast();
  const [orderData, setOrderData] = useState<DispatchScheduleData | null>(null);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (refreshError) {
        console.error("Error refreshing schedule data:", refreshError);
      ...
      toast({
        title: "Success",
        description: "Schedule draft saved successfully",
      });
    } catch (err: any) {
  ...
  const handleOrderItemsChange = (updatedItems: OrderItem[]) => {
    if (orderData) {
      ...
      console.log("Schedule items updated:", updatedItems);
    }
  };
  ...
  if (!orderData) return (
    <div className="flex justify-center items-center min-h-screen">
      <div className="text-gray-500">Schedule not found</div>
    </div>
  );
        ...
        <Card className="shadow-sm">
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle>Delivery Schedule #{orderData?.order_number}</CardTitle>
              {isSubmitted && (
              ...
              <div className="flex flex-col space-y-4">
                <div className="flex justify-end gap-4">
                  ...
                  >
                    ...
                    {isSubmitted ? "Resubmit Schedule" : "Submit Schedule"}
                  </Button>
                </div>
                ...
                <div className="flex justify-center pt-6 border-t">
                  ...
                  >
                    <Link to="/dispatch/archives" className="flex items-center gap-2">
